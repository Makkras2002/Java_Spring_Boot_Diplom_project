...
function changePage(page)
{
    ...
    for (let i = (page-1) * records_per_page; i < (page * records_per_page) && i < orders.length; i++) {
        ...
        for(let j =0; j < orders[i].clientsComponentOrders.length; j++) {
            components+="<tr><td>"+orders[i].clientsComponentOrders[j].product.productName +
                "</td><td>"+ orders[i].clientsComponentOrders[j].orderedProductAmount +" ед.</td><td>"+
                orders[i].clientsComponentOrders[j].orderedProductFullPrice + " BYN</td></tr>";
        }
        components+="</tbody></table>";
        tr +=
            "<form id='"+i+"' method='post' action='"+context+"/confirmClOrder'><input form='"+i+"' type='hidden' name='order_id' id='order_id"+i+"' required='required' readonly='readonly' value='"+
            orders[i].completeClientsOrderId +
            ...
            "</td>" +
            "<td>" +
            orders[i].completeClientsOrderDate.day+"."+orders[i].completeClientsOrderDate.month+"."+orders[i].completeClientsOrderDate.year+
            "</td>"+
            "<td>" +
            orders[i].deliveryAddress+
            "</td>"+
            "<td>" +
            orders[i].deliveryPrice+" BYN"+
            "</td>"+
