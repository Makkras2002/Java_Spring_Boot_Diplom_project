...
@Component
public class MonthlyScheduler implements CustomScheduler {
    private static final String MONTHLY_PERIOD = "0 0 9 1 1/1 ?";
    private static final Logger logger = LogManager.getLogger();
    private final CurrentFinancesService financesService;
    private final ExpensesService expensesService;

    @Autowired
    public MonthlyScheduler(CustomCurrentFinancesService financesService, CustomExpensesService expensesService) {
        this.financesService = financesService;
        this.expensesService = expensesService;
    }
    @Override
    @Scheduled(cron = MONTHLY_PERIOD)
    public void performFinancesRedactionBasedOnExpensesTask() {
        List<Expenses> expenses = expensesService.getAllExpensesByPeriod(PeriodType.MONTHLY);
